...
// @ts-ignore - postgres types are resolved at runtime
import postgres from "postgres";
import type { ArtifactKind, VisibilityType } from "./schema";
import {
  type Chat,
  chat,
  type DBMessage,
...
export function initDb(connectionString: string): PostgresJsDatabase {
...
export async function createUser(email: string, password: string): Promise<void> {
  ...
  await db.insert(user).values({ email, password: hashedPassword } as any);
}
...
export async function saveMessages({ messages }: { messages: DBMessage[] }): Promise<void> {
  const db = getDb();
  await db.insert(message).values(messages as any);
}
...
export async function saveSuggestions({
  ...
  const db = getDb();
  await db.insert(suggestion).values(suggestions as any);
}
...
export async function updateChatVisibilityById({
  ...
  const db = getDb();
  await db.update(chat).set({ visibility } as any).where(eq(chat.id, chatId));
}
