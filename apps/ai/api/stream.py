...
from api.asyncio import ValueEvent, wait_if_not_done
from api.command import map_cmd
from api.feature_flags import USE_DURABILITY, USE_RUNTIME_CONTEXT_API
from api.graph import get_graph
from api.js.base import BaseRemotePregel
from api.metadata import HOST, PLAN, USER_API_URL, incr_nodes
from api.schema import Run, StreamMode
from api.serde import json_dumpb
from api.utils.config import run_in_executor
...
def _preprocess_debug_checkpoint(
    ...
    from api.state import runnable_config_to_checkpoint
...
async def astream_state(
    ...
    *,
    ...
    config["metadata"]["api_version"] = __version__
    config["metadata"]["langgraph_plan"] = PLAN
    config["metadata"]["langgraph_host"] = HOST
    config["metadata"]["api_url"] = USER_API_URL
    # attach node counter
        ...
        try:
            async for mode, payload in stream:
                await Runs.Stream.publish(
                    ...
                    mode,
                    STREAM_CODEC.encode(
                        mode, await run_in_executor(None, json_dumpb, payload)
                    ),
                    thread_id=thread_id,
            ...
            await Runs.Stream.publish(
                ...
                "error",
                STREAM_CODEC.encode(
                    "error", await run_in_executor(None, json_dumpb, e)
                ),
                thread_id=thread_id,
            )
            raise e
