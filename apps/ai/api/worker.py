...
import api.logging as lg_logging
from api.auth.custom import SimpleUser, normalize_user
from api.config import (
    BG_JOB_ISOLATED_LOOPS,
    BG_JOB_MAX_RETRIES,
    BG_JOB_TIMEOUT_SECS,
)
from api.errors import UserInterrupt, UserRollback, UserTimeout
from api.js.errors import RemoteException
from api.metadata import incr_runs
from api.schema import Run, StreamMode
from api.state import state_snapshot_to_thread_state
from api.stream import AnyStream, astream_state, consume
from api.utils import with_user
from storage.database import connect
from storage.ops import Runs, Threads
from storage.retry import RETRIABLE_EXCEPTIONS
    ...
    async with Runs.enter(run_id, run["thread_id"], main_loop, resumable) as done:
        ...
        async with connect() as conn:
            ...
            if temporary and not isinstance(exception, ALL_RETRIABLE_EXCEPTIONS):
                # Delete the thread (this should cascade delete the run)
