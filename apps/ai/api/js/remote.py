...
from api.auth.custom import DotDict, ProxyUser
from api.config import LANGGRAPH_AUTH, LANGGRAPH_AUTH_TYPE
from api.js.base import BaseRemotePregel, RemoteInterrupt
from api.js.errors import RemoteException
from api.js.sse import SSEDecoder, aiter_lines_raw
from api.route import ApiResponse
...
class CustomJsAuthBackend(AuthenticationBackend):
    ...
    def __init__(self, disable_studio_auth: bool = False):
        from api.utils.cache import LRUCache

        self.ls_auth = None
        if not disable_studio_auth and LANGGRAPH_AUTH_TYPE == "langsmith":
            from api.auth.langsmith.backend import LangsmithAuthBackend
