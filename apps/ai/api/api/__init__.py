...
from api.api.a2a import a2a_routes
from api.api.assistants import assistants_routes
from api.api.mcp import mcp_routes
from api.api.meta import meta_info, meta_metrics
from api.api.openapi import get_openapi_spec
from api.api.runs import runs_routes
from api.api.store import store_routes
from api.api.threads import threads_routes
from api.api.ui import ui_routes
from api.auth.middleware import auth_middleware
from api.config import HTTP_CONFIG, MIGRATIONS_PATH, MOUNT_PREFIX
from api.graph import js_bg_tasks
from api.js.base import is_js_path
from api.validation import DOCS_HTML
from storage.database import connect, healthcheck
...
async def ok(request: Request, *, disabled: bool = False):
    ...
    if check_db:
        await healthcheck()
    if js_bg_tasks:
        from api.js.remote import js_healthcheck
...
def load_custom_app(app_import: str) -> Starlette | None:
    ...
    # we are handling this in `api.js.remote.JSCustomHTTPProxyMiddleware`
    if is_js_path(path):
        return None
...
if "inmem" in MIGRATIONS_PATH:
    ...
    async def truncate(request: Request):
        from api.checkpoint import Checkpointer
