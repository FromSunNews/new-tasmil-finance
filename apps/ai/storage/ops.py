...
from typing import TYPE_CHECKING, Any, AsyncContextManager, Literal, cast  # noqa: UP035
...
from api.asyncio import SimpleTaskGroup, ValueEvent, create_task
from api.auth.custom import handle_event
from api.config import BG_JOB_HEARTBEAT, BG_JOB_INTERVAL
...
from api.schema import (
    Interrupt as InterruptSchema,
)
from api.serde import Fragment, ajson_loads
from api.state import state_snapshot_to_thread_state
from api.utils import fetchone, get_auth_ctx, next_cron_date
from api.utils.stream_codec import decode_stream_message

# from langgraph.checkpoint.postgres.aio import AsyncPostgresSaver
from storage.async_postgres_checkpointer import (
    AsyncPostgresCheckpointer as AsyncPostgresSaver,
)
...
class Assistants(Authenticated):
    ...
    async def search(
        ...
        *,
        ...
        select_clause = "*"
        if select:
            # Sanitize select columns to prevent SQL injection
            select_clause = ", ".join(select)

        # Build base query for counting total
        count_query = """SELECT COUNT(*) FROM assistant
        WHERE graph_id = ANY(%(graph_ids)s) AND metadata @> %(metadata)s"""
        query = f"""SELECT {select_clause} FROM assistant
        WHERE graph_id = ANY(%(graph_ids)s) AND metadata @> %(metadata)s"""
        ...
        if graph_id:
            ...
            count_query += " AND graph_id = %(graph_id)s"
        ...
        total = await count_cur.fetchone()
        total_count = total["count"] if total else 0

        return (row async for row in cur), total_count
    ...
    async def count(
        ...
        *,
        ...
        """Count assistants matching the given criteria."""
        metadata = metadata if metadata is not None else {}
        filters = await Assistants.handle_event(
            ctx,
            "search",
        ...
        count_cur = await conn.execute(count_query, params, binary=True)
        total = await count_cur.fetchone()
    ...
    async def put(
        ...
        *,
        ...
        inserted_version as (
            INSERT INTO assistant_versions (assistant_id, graph_id, config, context, metadata, version, description)
            SELECT assistant_id, graph_id, config, context, metadata, 1 as version, description
        ...
        params = {
            ...
            "graph_id": graph_id,
            "config": Jsonb(config),
            "context": Jsonb(context if context is not None else {}),
...
class Threads(Authenticated):
    ...
    async def search(
        ...
        *,
        ...
        # Build base query for counting total
        count_query = "SELECT COUNT(*) FROM thread"
        query = f"SELECT {select_clause} FROM thread"
        ...
        if ids:
            ...
            where_clauses.append("thread_id = ANY(%(ids)s)")
            params["ids"] = uuid_ids
        if metadata:
        ...
        if sort_by:
            # Validate sort_by to prevent SQL injection
            valid_sort_fields = ["thread_id", "status", "created_at", "updated_at"]
            if sort_by in valid_sort_fields:
                sort_direction = "ASC" if sort_order == "asc" else "DESC"
                query += f" ORDER BY {sort_by} {sort_direction}"
            else:
    ...
    async def count(
        ...
        *,
        ...
        """Count threads matching the given criteria."""
        metadata = metadata if metadata is not None else {}
        values = values if values is not None else {}
        filters = await Threads.handle_event(
            ctx,
    ...
    async def put(
        ...
        *,
        ...
        # TODO: Implement TTL support once database schema supports TTL columns
        # For now, we accept the ttl parameter but don't use it
        metadata = metadata if metadata is not None else {}
        filters = await Threads.handle_event(
            ctx,
            "create",
    ...
    async def set_joint_status(
        ...
        async with conn.transaction():
            # Handle run status update or deletion
            if run_status == "rollback":
                # Delete the run
                await conn.execute(
                    "DELETE FROM run WHERE run_id = %(run_id)s AND thread_id = %(thread_id)s",
                    {"run_id": run_id, "thread_id": thread_id},
                )
            else:
                # Update run status
                await conn.execute(
                    "UPDATE run SET status = %(status)s, updated_at = now() WHERE run_id = %(run_id)s AND thread_id = %(thread_id)s",
                    {"status": run_status, "run_id": run_id, "thread_id": thread_id},
                )

            # Update thread status - the SQL query determines the final status based on active runs
            await conn.execute(
                """UPDATE thread SET
                updated_at = now(),
                values = %(values)s,
                interrupts = %(interrupts)s,
                metadata = metadata || %(graph_metadata)s,
                status = case
                    when exists(
                        select 1 from run
                        where thread_id = %(thread_id)s
                ...
                WHERE thread_id = %(thread_id)s""",
                {
                    ...
                    "values": Jsonb(checkpoint["values"]) if checkpoint else None,
                    "interrupts": Jsonb(interrupts),
                    "graph_metadata": Jsonb({"graph_id": graph_id}),
                    "base_status": base_thread_status,
                },
                binary=True,
            )
    ...
    async def sweep_ttl(
        ...
        Returns tuple of (threads_processed, threads_deleted).
        Currently a stub implementation since TTL is not fully implemented.
        """
        # TODO: Implement TTL sweeping once database schema supports TTL columns
        # For now, return 0 to indicate no threads were processed or deleted
        return (0, 0)
        ...
        async def list(
            ...
            *,
            ...
            if graph_id := thread_metadata.get("graph_id"):
                async with get_graph(
                    graph_id, thread_config, checkpointer=AsyncPostgresSaver(conn)
                ) as graph:
                    result = [
                        c
...
SELECT * FROM inflight_runs"""
    ...
    async def cancel(
        ...
        *,
        ...
        logger.info("Canceling runs", run_ids=run_ids)
        filters = await Runs.handle_event(
            ctx,
            "update",
    ...
    class Stream(Authenticated):
        ...
        async def check_run_stream_auth(
            ...
            *,
            ...
            """Check authorization to access the run stream."""
