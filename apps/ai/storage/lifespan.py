...
from storage.database import start_pool, stop_pool
...
async def lifespan(
    ...
    **kwargs: Any,
    ...
    from api import feature_flags, graph, thread_ttl
    from api import store as api_store
    from api.asyncio import SimpleTaskGroup, set_event_loop
    from api.http import start_http_client, stop_http_client
    from api.js.ui import start_ui_bundler, stop_ui_bundler
    from api.metadata import metadata_loop

    await logger.ainfo(
        f"Starting In-Memory runtime with langgraph-api={api_version}",
    )
    try:
